...
class LocalCoherenceTensor:
    def evaluate(self, state, memory_state, neighbor_proj: list[float], world_drive: float) -> dict[str, float]:
        ...
        tissue = state.tissue
        ctx = state.context
        dyn_norm = math.sqrt(sum(x * x for x in state.dynamic)) / max(1.0, len(state.dynamic) ** 0.5)
        neigh_norm = math.sqrt(sum(x * x for x in neighbor_proj)) / max(1.0, len(neighbor_proj) ** 0.5)
        ...
        C_neighbors = _clip01(1.0 - abs(dyn_norm - neigh_norm))
