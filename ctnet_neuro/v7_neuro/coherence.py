...
class LocalCoherenceTensor:
    def evaluate(self, state, memory_state, neighbor_proj: float, world_drive: float) -> dict[str, float]:
        body = state.body
        tissue = state.tissue
        ctx = state.context
        ...
        neighbor_mean = sum(neighbor_proj)/max(1,len(neighbor_proj))
        local_mean = sum(state.dynamic)/max(1,len(state.dynamic))
        C_neighbors = _clip01(1.0 - abs(local_mean - neighbor_mean))
